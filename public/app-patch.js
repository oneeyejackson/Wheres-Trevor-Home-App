(()=>{
  ...
  function forceHomePhoto(){document.querySelectorAll('.city-photo.home-photo').forEach(el=>{el.style.setProperty('background-image',`linear-gradient(135deg,#15201466,#05080522),url('${HOME_HERO}')`,'important');el.style.setProperty('background-size','cover','important');el.style.setProperty('background-position','center','important')})}
  function prunePastSchedule(){const today=todayLA();document.querySelectorAll('.expanded-day[id^="day-"]').forEach(box=>{const date=box.id.replace('day-','');if(date<today){const wrap=box.closest('.schedule-row-wrap');if(wrap)wrap.remove()}})}
  function localRefresh(){forceHomePhoto();prunePastSchedule();}
  ...
  const mo=new MutationObserver(()=>{const menu=document.querySelector('#userMenu .row');if(menu&&!menu.querySelector('.guest-lock-button')){const b=document.createElement('button');b.textContent='Guest mode';b.className='guest-lock-button';b.onclick=showGuestLock;menu.insertBefore(b,menu.lastElementChild)}localRefresh();});
  ...
  async function softRefresh(reason){
    ...
    const now=Date.now();
    const minGap = reason==='visible' ? FOCUS_REFRESH_MS : NETWORK_REFRESH_MS;
    if(now-lastRefresh<minGap) return;
    lastRefresh=now;
    try{await window.load()}catch{}
  }
  setInterval(()=>softRefresh('timer'),NETWORK_REFRESH_MS);
  setInterval(()=>{if(localStorage.getItem('wt_guest_lock')==='1')showGuestLock()},GUEST_REFRESH_MS);
  setInterval(localRefresh,LOCAL_REFRESH_MS);
